"""Backend fix agent — generates code changes for API routes and configs."""
...
def _is_backend_file(path: str) -> bool:
    """Determine if a file path belongs to the backend domain."""
    api_dirs = ("pages/api/", "app/api/", "src/pages/api/", "src/app/api/", "backend/", "routes/", "controllers/", "api/")
    if any(d in path for d in api_dirs):
        return True
        
    configs = ("next.config.", "vercel.json", "middleware.ts", "src/middleware.ts", "server.js", "package.json")
    if any(path.startswith(c) or path == c or path.endswith("/" + c) for c in configs):
        return True
...
async def run(state: PipelineState) -> dict[str, Any]:
    """Execute the backend code generation phase."""
    run_id = state.get("run_id", "unknown")
    start_time = time.monotonic()
    sse = get_publisher()

    try:
        await sse.publish_agent_start(run_id, "backend_fix_agent", "Generating backend code changes")

        fix_plan = state.get("fix_plan", {})
        all_files_to_change = fix_plan.get("files_to_change", [])
        
        # Filter to backend domain
        target_files = [f for f in all_files_to_change if _is_backend_file(f.get("path", ""))]

        if not target_files:
            duration_ms = int((time.monotonic() - start_time) * 1000)
            await sse.publish_agent_complete(run_id, "backend_fix_agent", "No backend files to change", duration_ms)
            return {
                "agent_steps": [{
                    "agent": "backend_fix_agent",
                    "status": "complete",
                    "summary": "No backend files to change",
                    "duration_ms": duration_ms
                }],
                "current_agent": "backend_fix_agent"
            }
...
"""
            ...
            try:
                logger.info(f"Generating backend fix for {path}")
        ...
        duration_ms = int((time.monotonic() - start_time) * 1000)
        summary = f"Generated {len(generated_fixes)} backend file fixes"
        await sse.publish_agent_complete(run_id, "backend_fix_agent", summary, duration_ms)
        ...
        logger.exception("Backend fix agent failed")
        duration_ms = int((time.monotonic() - start_time) * 1000)
        await sse.publish_agent_error(run_id, "backend_fix_agent", str(exc), duration_ms)
        
        return {
            "error_log": [f"backend_fix_agent error: {exc}"],
            "agent_steps": [{
                "agent": "backend_fix_agent",
                "status": "error",
